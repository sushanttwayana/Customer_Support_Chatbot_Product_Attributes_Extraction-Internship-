...
class CustomerSupportChatbot:
    ...
    def save_conversation_to_csv(self, session_id: str, user_question: str, bot_response: str, ip_address: str, user_agent: str):
        """Save user question and bot response to a CSV file"""
        try:
            file_exists = os.path.isfile('conversations.csv')
            with open('conversations.csv', 'a', newline='', encoding='utf-8') as csvfile:
                writer = csv.writer(csvfile, quoting=csv.QUOTE_MINIMAL)
                if not file_exists:
                    # Write header if file is new
                    writer.writerow(['timestamp', 'session_id', 'user_question', 'bot_response', 'ip_address', 'user_agent'])
            ...
            logger.info(f"Saved conversation for session {session_id} to conversations.csv")
        except Exception as e:
            logger.error(f"Failed to save conversation to CSV: {str(e)}")
